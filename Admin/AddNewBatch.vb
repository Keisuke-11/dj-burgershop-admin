...
Public Class AddNewBatch
    ...
    Private Sub AddBatchToDatabase()
        ...
        Try
            ...
            Try
                ' Generate a batch number: BATCH-YYYYMMDD-HHmmss
                Dim batchNumber As String = "BATCH-" & DateTime.Now.ToString("yyyyMMdd-HHmmss")
                ...
                Dim insertSQL As String =
                    ...
                    "(IngredientID, BatchNumber, StockQuantity, OriginalQuantity, UnitType, CostPerUnit, TotalCost, " &
                    "PurchaseDate, ExpirationDate, StorageLocation, BatchStatus, Notes) " &
                    "VALUES (@ingredientID, @batchNumber, @stockQty, @origQty, @unitType, @costPerUnit, @totalCost, " &
                    "@purchaseDate, @expirationDate, @storageLocation, 'Active', @notes)"

                Dim cmdBatch As New MySqlCommand(insertSQL, conn, transaction)
                cmdBatch.Parameters.AddWithValue("@ingredientID", _ingredientID)
                cmdBatch.Parameters.AddWithValue("@batchNumber", batchNumber)
                cmdBatch.Parameters.AddWithValue("@stockQty", quantity)
                cmdBatch.Parameters.AddWithValue("@origQty", quantity)
                cmdBatch.Parameters.AddWithValue("@unitType", unit)
                cmdBatch.Parameters.AddWithValue("@costPerUnit", costPerUnit)
                cmdBatch.Parameters.AddWithValue("@totalCost", totalCost)
                cmdBatch.Parameters.AddWithValue("@purchaseDate", purchaseDate)
                cmdBatch.Parameters.AddWithValue("@expirationDate", If(expirationDate = Date.MinValue, DBNull.Value, CObj(expirationDate)))
                cmdBatch.Parameters.AddWithValue("@storageLocation", storageLocation)
                cmdBatch.Parameters.AddWithValue("@notes", fullNotes)
