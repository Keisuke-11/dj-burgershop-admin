...
Public Class Dashboard
    ...
    Private Sub Dashboard_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ' Hide Catering legend row — feature not in use, keep data intact
        Label12.Visible = False          ' "Catering" text label
        lblPercentCatering.Visible = False
        lblValueCatering.Visible = False
        Panel3.Visible = False           ' Catering colour swatch
        ...
        Try
            ...
            cmd = New MySqlCommand($"
                SELECT COALESCE(
                    ...
                    (SELECT SUM(AmountPaid) FROM reservationpayment WHERE ReservationID IS NOT NULL AND PaymentStatus IN ('Paid', 'Completed') AND {dateFilterPayment}),
                    0
        ...
        Try
            ...
            cmd = New MySqlCommand($"
                SELECT (
                    ...
                    SELECT COUNT(DISTINCT ReservationID) FROM reservationpayment 
    ...
    Private Sub Reservations_Click(sender As Object, e As EventArgs)
        Try
            ...
            End If
        Catch ex As Exception
            MessageBox.Show("Error navigating to reservation screen: " & ex.Message)
        End Try
    End Sub
...
LIMIT 8;"
        ...
        cmd = New MySqlCommand($"
                ...
                FROM (
                    SELECT 
                        ...
                        (SELECT COALESCE(SUM(AmountPaid), 0) FROM reservationpayment WHERE ReservationID IS NOT NULL AND PaymentStatus IN ('Paid', 'Completed') AND {dateFilterPayment}) as TotalRev
                ) as revs,
                (
                    SELECT 
                        (SELECT COUNT(*) FROM orders WHERE OrderStatus IN ('Completed', 'Served') AND {dateFilterOrder}) + 
                        (SELECT COUNT(DISTINCT ReservationID) FROM reservationpayment WHERE ReservationID IS NOT NULL AND PaymentStatus IN ('Paid', 'Completed') AND {dateFilterPayment}) as TotalCount
                ) as counts", conn)
    ...
    Private Sub LoadSalesByChannel()
        ...
        AND {paymentDateFilter}", conn)
            ...
            ' Merge catering revenue into takeout/online so the chart still balances.
            ' Catering slice is hidden from the UI; its revenue is not lost from totals.
            Dim combinedTakeout As Decimal = takeoutSales + onlineSales + cateringRevenue

            If totalSales > 0 Then
                Dim dineInPercent As Decimal = (dineInSales / totalSales) * 100
                Dim takeoutPercent As Decimal = (combinedTakeout / totalSales) * 100

                ' Add data points — Catering is NOT added as a separate slice
                ...
                point2.SetValueXY("Takeout", combinedTakeout)
                ...
                ' Takeout label includes catering + online (hidden from user)
                lblPercentTakeout.Text = Math.Round(takeoutPercent, 0).ToString() & "%"
                lblValueTakeout.Text = "₱" & combinedTakeout.ToString("N2")
            Else
    ...
    Private Sub LoadInventoryAlerts()
        Try
            If Me.Controls.Contains(InventoryAlerts) Then
                ...
                InventoryAlerts.Controls.Clear() ' ✅ FIX: Clear old alerts before adding new ones
            End If
            openConn()
            ...
            SELECT 
                ...
                CASE 
                    ...
                    WHEN COALESCE(SUM(ib.StockQuantity), 0) < i.ReorderLevel THEN 'Low'
                    ELSE 'OK'
                END AS 'Alert Level'
            FROM ingredients i
            ...
            GROUP BY i.IngredientID, i.IngredientName, i.UnitType, i.ReorderLevel
            HAVING COALESCE(SUM(ib.StockQuantity), 0) < i.ReorderLevel
            ORDER BY 
                CASE 
    ...
    Private Sub ConfigureChartTopProducts()
        ...
        tooltip.SetToolTip(Chart2, "Click to view detailed Catering reservation report")
    End Sub
    ...
    Private Function BuildSalesQueryForDashboard(currentYear As Integer) As String
        ...
        If TableExists("order") OrElse TableExists("payment") Then
            queries.Add($"
    ...
    Private Sub ProductPerformance_Click(sender As Object, e As EventArgs)
        ...
        Catch ex As Exception
            MessageBox.Show("Error navigating to catering reservation: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
