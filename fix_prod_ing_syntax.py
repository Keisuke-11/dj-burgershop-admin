...
for root, _, files in os.walk(project_dir):
    for file in files:
        if file.endswith(".vb"):
            ...
            # Match FROM/JOIN/INTO/UPDATE followed by optional backtick, 'product', optional backtick, then space/quote/newline
            replace_regex_in_file(filepath, r'(FROM|JOIN|INTO|UPDATE)\s+`?product`?\b(?!s|_)', r'\1 products')
            replace_regex_in_file(filepath, r'(FROM|JOIN|INTO|UPDATE)\s+`?ingredient`?\b(?!s|_)', r'\1 ingredients')
